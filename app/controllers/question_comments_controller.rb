...
class QuestionCommentsController < AnonCommentController
  ...
  def create
    ...
    respond_to do |format|
      if Rails.env.production? 
        @question_comment=verify_recaptcha(:model => @question_comment, :message => "Oh! It's error with reCAPTCHA!") && @question_comment.question_comment.create(question_comment_params)
      else 
  ...
  def question_comment_params
    params.require(:question_comment).permit(:id,:title,:name,:password,:content)
  end
end
