...
class ContactsController < AnonBoardController
  ...
  # GET /contacts.json
  def index
    @contacts = Contact.order(@menu_setting.order).page(params[:page]).per(@menu_setting.per)
  ...
  def contact_params
    params.require(:contact).permit(:id, :name, :phone, :email, :title, contact_content_attributes: [:id,:content])
  end  
end
