...
class Admin::ResourcesController < Admin::AdminController
  ...
  def show
    @admin_resources = Resource.find(params[:id])
  ...
  def new
    @admin_resources = Resource.new
    
    respond_to do |format|
      format.html # new.html.erb
      format.json { render json: @admin_resources }
    end
  end
  ...
  def create
    @admin_resources = Resource.new(params[:menu])

    respond_to do |format|
      if @admin_resources.save
        format.html { redirect_to admin_menus_url, notice: '공지사항이 작성되었습니다.' }
        format.json { render json: @admin_resources, status: :created, location: @admin_resources }
      else
        format.html { render action: "new" }
        format.json { render json: @admin_resources.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      if @admin_resources.update_attributes(params[:menu])
